...
// import { ACLService } from 'yoyo-ng-module/src/acl';
// import { ALAIN_I18N_TOKEN, AlainI18NService } from 'yoyo-ng-module/src/theme';
import { deepCopy } from 'yoyo-ng-module/src/util';
import { LocalizationService, PermissionCheckerService } from 'yoyo-ng-module/src/abp';
...
export class STColumnSource {
  ...
  private btnCoerce(list: STColumnButton[]): STColumnButton[] {
    ...
    for (const item of list) {
      if (this._permissionService && item.acl && !this._permissionService.isGranted(item.acl)) {
        continue;
      }
      ...
      // }
      if (item.i18n && this._localizationService) {
        item.text = this._localizationService.l(item.i18n);
      }
  ...
  private filterCoerce(item: STColumn): STColumnFilter {
    ...
    if (this._permissionService) {
      res.menus = res.menus.filter(w => this._permissionService.isGranted(w.acl));
    }
  ...
  process(list: STColumn[]): STColumn[] {
    ...
    for (const item of copyColumens) {
      ...
      // }
      if (item.i18n && this._localizationService) {
        item.title = this._localizationService.l(item.i18n);
      }
      // no
      if (item.type === 'no') {
      ...
      // }
      if (this._permissionService) {
        item.selections = item.selections.filter(w => this._permissionService.isGranted(w.acl));
      }
      // radio
      if (item.type === 'radio') {
        ++radioCount;
