...
yoyo-ng-module/form 是一个基于 [JSON Schema](http://json-schema.org/) 标准的动态构建表单。
...
安装 `yoyo-ng-module/form` 依赖包：

```bash
npm install yoyo-ng-module/form --save
```
...
虽然默认 `yoyo-ng-module/form` 校验是 [ajv](http://epoberezkin.github.io/ajv/)，但这并不是唯一的选择，你可以覆盖 `SchemaValidatorFactory` 使用其他校验类库，所以 `ajv` 并没有强制依赖；其实从另一个点来看将这种第三方类库统一在 `scripts` 加载才是更合理的，因此你依然要自己安装 `ajv` 并在 `angular.json` 中导入它：
